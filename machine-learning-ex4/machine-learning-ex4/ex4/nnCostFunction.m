...
% j
% J = 1/m * j;
% [~,h] = max([ones(m,1) [ones(m,1) X] * Theta1'] * Theta2',[],2);
% J = (y' * log(sigmoid(h)) + (1 - y)'*log(1-sigmoid(h))) *(-1/m) ;
...
Delta1 = zeros(size(Theta1)) + delta2' * a1;%25x401
Delta2 = zeros(size(Theta2)) + delta3' * a2;%10x26


Theta1_grad = 1/m * Delta1;   
Theta2_grad = 1/m * Delta2;

regularized_1 = lambda / m .* [zeros(size(Theta1,1),1) Theta1(:,2:end)];
regularized_2 = lambda / m .* [zeros(size(Theta2,1),1) Theta2(:,2:end)];
